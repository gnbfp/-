...
2. 找不到的字段填空字符串；weight 找不到填 null。绝不编造。
3. 无法客观核对的要求（如“内容充实”“排版美观”）→ status 填 "ambiguous"；
   其余填 "normal"。
4. id 从 R1 起连续编号，不要重复。
5. deadline 用 "YYYY-MM-DDTHH:MM"；原文没有明确截止时间就留空字符串。
6. 只做解析：不要拆任务、不要提改进建议、不要评价作业书。"""
...
def _validate_points(raw, text: str, problems: list[str]) -> list[RubricPoint]:
    if not isinstance(raw, list) or not raw:
        problems.append("rubric 必须是非空数组")
        return []
